...
const OUTCOME_CHIP: Record<string, string> = {
  ...
  immature: "bg-sky-500/15 text-sky-300",
...
export function BacktestPanel({ refreshKey }: { refreshKey: number }) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(setData)
      .catch((e: unknown) =>
        setError(e instanceof Error ? e.message : "Unable to load backtest"),
      )
      .finally(() => setLoading(false));
  }, [refreshKey]);

  if (loading && !data) {
    return <p className="mt-8 text-sm text-slate-500">Backtesting past verdicts…</p>;
  }
  if (error) {
    return <p className="mt-8 text-sm text-rose-400">Backtest unavailable: {error}</p>;
  }
  if (!data) return null;

  const { summary, entries } = data;
  const scoredEntries = entries.filter((e) => e.outcome === "hit" || e.outcome === "miss");

  return (
    <section className="mt-6">
      <div className="mb-4">
        <h2 className="text-lg font-semibold text-slate-100">Verdict backtest</h2>
        <p className="mt-0.5 text-xs text-slate-400">
          Each verdict graded at the horizon it actually committed to — not just “since
          issued.” {summary.rule}
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          label="Hit rate at horizon"
          value={summary.hit_rate === null ? "—" : `${(summary.hit_rate * 100).toFixed(0)}%`}
          hint={summary.scored ? `${summary.hits} of ${summary.scored} matured verdicts` : "no matured verdicts yet"}
        />
        <StatCard
          label="Avg return"
          value={
          ...
          hint="across matured verdicts"
        />
        <StatCard label="Matured" value={String(summary.scored)} hint="horizon has elapsed" />
        <StatCard label="Still open" value={String(summary.immature)} hint="horizon not yet reached" />
      </div>

      {summary.by_horizon.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2">
          {summary.by_horizon.map((h) => (
            ...
            >
              <span className="font-semibold text-slate-100">{h.horizon_days}d</span>{" "}
              {h.hit_rate === null ? "—" : `${(h.hit_rate * 100).toFixed(0)}% hit`}
      ...
      {scoredEntries.length === 0 ? (
        <p className="mt-6 text-sm text-slate-500">
          No verdicts have reached their horizon yet — check back once some mature.
        </p>
      ) : (
          ...
          <table className="w-full min-w-[720px] text-left text-xs">
            <thead className="bg-slate-900 text-[10px] uppercase tracking-wider text-slate-500">
              <tr>
                ...
                <th className="px-3 py-2.5 text-right">At horizon</th>
            ...
            <tbody className="divide-y divide-slate-800/80 bg-slate-950/40">
              {scoredEntries.map((e) => (
                <tr key={e.id} className="hover:bg-slate-900/50">
                  ...
                  <td className="px-3 py-2 text-slate-400">{e.horizon_days}d</td>
                  <td className="px-3 py-2 text-slate-400">{toDate(e.created_at)}</td>
                  <td className="px-3 py-2 text-slate-400">
                    {e.evaluated_at ? toDate(e.evaluated_at) : "—"}
                  </td>
                  <td className="px-3 py-2 text-right text-slate-300">
                    {e.price_at_run === null ? "—" : `$${e.price_at_run.toFixed(2)}`}
                  </td>
                  <td className="px-3 py-2 text-right text-slate-300">
                    {e.price_at_horizon === null ? "—" : `$${e.price_at_horizon.toFixed(2)}`}
                  </td>
                  <td
